...
df1 = pd.read_csv("var_drug_ann_model2.csv", sep=';', usecols=['Drug', 'Genotype', 'Outcome', 'Variation'])
df2 = pd.read_csv("var_fa_ann_model.csv", sep=';', usecols=['Drug', 'Genotype', 'Outcome', 'Variation'])
df3 = pd.read_csv("var_pheno_ann_model.csv", sep=';', usecols=['Drug', 'Genotype', 'Outcome', 'Variation'])

genes_df = pd.concat([df1['Genotype'], df2['Genotype'], df3['Genotype']]).dropna().unique().tolist()


df = pd.concat([df1, df2, df3], ignore_index=True,)  # columnas: text1, text2
