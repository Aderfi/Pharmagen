...
def extract_vocab(json_file):
    ...
    with open(
        Path((cfg.MODEL_LABEL_VOCABS_DIR) / f"{json_file_base}_vocab.json"),
        "w",
        encoding="utf-8",
    ) as f:
...
vocab_files = [
    f for f in os.listdir(cfg.MODEL_LABEL_VOCABS_DIR) if f.endswith("_vocab.json")
]
vocab1 = set()
...
full_vocab = set().union(
    *[extract_vocab(json_dir / json_files[i]) for i in range(len(json_files))]
)
...
def get_embedding(token):
    with torch.no_grad():
        inputs = tokenizer(
            token, return_tensors="pt", truncation=True, max_length=32, verbose=True
        )
