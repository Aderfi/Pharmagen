...
outcome_variation_pairs = list(zip(df['Outcome'].astype(str), df['Variation'].astype(str)))
variation_variation1_pairs = list(zip(df['Variation'].astype(str), df['Variation_1'].astype(str)))
variation1_effect_pairs = list(zip(df['Variation_1'].astype(str), df['Outcome'].astype(str)))  # Cambia 'Outcome' si tienes otra columna
...
outcome_variation_embeddings = get_biobert_embeddings(outcome_variation_strs, tokenizer, model, device)
variation_variation1_embeddings = get_biobert_embeddings(variation_variation1_strs, tokenizer, model, device)
variation1_effect_embeddings = get_biobert_embeddings(variation1_effect_strs, tokenizer, model, device)
...
drug_gene_strs = [f"{row['Drug']} [SEP] {get_genes(row['Drug'])}" for _, row in df.iterrows()]
