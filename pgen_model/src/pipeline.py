...
def train_pipeline(
    ...
    csv_path=Path(MODEL_TRAIN_DATA,  "final_test_genalle.tsv")
    ...
    df = processor.load_data(
        ...
        cols_to_use=list(set(config["cols"] + config["targets"])),
    ...
    train_df, val_df = train_test_split(df, test_size=0.2, stratify=df['stratify_col'], random_state=seed)
    
    processor.fit(train_df)
    train_enc = processor.transform(train_df)
    val_enc = processor.transform(val_df)
    
    train_ds = PGenDataset(train_enc, feature_cols, target_cols, MULTI_LABEL_COLUMN_NAMES)
    val_ds = PGenDataset(val_enc, feature_cols, target_cols, MULTI_LABEL_COLUMN_NAMES)
    
    train_loader = DataLoader(train_ds, batch_size=config["params"]["batch_size"], shuffle=True, num_workers=4, pin_memory=True)
    val_loader = DataLoader(val_ds, batch_size=config["params"]["batch_size"], shuffle=False, num_workers=4, pin_memory=True)
    
    # Dimensiones
    n_features = {}
    target_dims = {}
    for col, enc in processor.encoders.items():
        if col in feature_cols: n_features[col] = len(enc.classes_)
        if col in target_cols: target_dims[col] = len(enc.classes_)
    ...
    best_loss, best_acc, task_losses = train_model(
        ...
        epochs=epochs, patience=20, model_name=model_name,
        feature_cols=feature_cols, target_cols=target_cols,
        device=device, scheduler=scheduler, 
        multi_label_cols=MULTI_LABEL_COLUMN_NAMES,
        return_per_task_losses=True, progress_bar=True
    )
    
    # Guardar
    save_model(model, target_cols, best_loss, best_acc, task_losses, model_name, params)
