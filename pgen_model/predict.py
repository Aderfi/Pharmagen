...
def predict_single_input(drug, gene, allele, genotype, model, encoders, target_cols):
    ...
    try:
        drug_tensor = torch.tensor(
            [encoders["drug"].transform([drug])[0]], dtype=torch.long, device=device
        )
        gene_tensor = torch.tensor(
            [encoders["gene"].transform([gene])[0]], dtype=torch.long, device=device
        )
        allele_tensor = torch.tensor(
            [encoders["allele"].transform([allele])[0]], dtype=torch.long, device=device
        )
        geno_tensor = torch.tensor(
            [encoders["genotype"].transform([genotype])[0]],
            dtype=torch.long,
            device=device,
        )
    except Exception as e:
        print(
            f"Error al transformar inputs. ¿Están las categorías en los encoders? {e}"
        )
        return None
...
def predict_from_file(file_path, model, encoders, target_cols):
    ...
    try:
        drug_tensor = torch.tensor(
            encoders["drug"].transform(df["drug"].astype(str)),
            dtype=torch.long,
            device=device,
        )
        gene_tensor = torch.tensor(
            encoders["gene"].transform(df["gene"].astype(str)),
            dtype=torch.long,
            device=device,
        )
        allele_tensor = torch.tensor(
            encoders["allele"].transform(df["allele"].astype(str)),
            dtype=torch.long,
            device=device,
        )
        geno_tensor = torch.tensor(
            encoders["genotype"].transform(df["genotype"].astype(str)),
            dtype=torch.long,
            device=device,
        )
    except Exception as e:
