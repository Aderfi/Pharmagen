...
import numpy as np
import pandas as pd
from src.data_handler import PGenProcessor, PGenDataset, DataConfig
...
def test_processor_fit_transform(sample_df):
    ...
    assert tensors["phenotype"].dtype == torch.float32 # BCE expects floats
    assert tensors["phenotype"].dim() == 2 # (Batch, Classes)
...
def test_unknown_handling(sample_df):
    ...
    # Should transform to UNKNOWN token index
    # We need to find what the index is to verify
