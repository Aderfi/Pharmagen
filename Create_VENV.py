...
def check_python_version(version):
    ...
    try:
        ...
        print(f"❌ La versión de Python {version} no se encuentra. Por favor, instálala.")
        return False
...
def install_requirements(env_name, requirements_file):
    ...
    try:
        ...
        pip_path = f"{env_name}/bin/pip" if sys.platform != "win32" else f"{env_name}/Scripts/pip.exe"
        
        subprocess.run([pip_path, "install", "-r", requirements_file], check=True)
        print("✅ Todas las dependencias se instalaron correctamente.")
        return True
    except FileNotFoundError:
        print(f"❌ No se pudo encontrar el archivo de requerimientos: {requirements_file}")
        return False
