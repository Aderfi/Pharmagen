...
def generate_training_report(history: list[dict[str, Any]], output_dir: Path, model_name: str):
    ...
    try:
        ...
        print(f"Warning: Could not generate HTML report (matplotlib missing?): {e}")
...
def _create_html_report(history: list[dict[str, Any]], output_dir: Path, model_name: str):
    ...
    # Filter columns that look like "val_acc_TASKNAME"
    # We exclude "val_acc_macro" to focus on specific tasks if desired, or include it.
...
def _generate_table_rows(df, acc_cols):
    rows = ""
    # Show last 10 epochs + best epoch? Or all? Let's show all for now, assuming < 100 epochs.
